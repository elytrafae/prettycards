...
function GenerateBuyRow(pack_data) {
	return `
		<input type="number" class="PrettyCards_PackBuyCount" pack_id="${pack_data.code_id}" value="1" min="1" pattern="[0-9]">
		x <button class="PrettyCards_PackGBuy" pack_id="${pack_data.code_id}">Buy (<span id="PrettyCards_PackGPrice" pack_id="${pack_data.code_id}">100</span> <img src="images/icons/gold.png" class="height-16">)</button>
		 <button class="PrettyCards_PackUcpBuy" pack_id="${pack_data.code_id}">Buy (<span class="ucp" id="PrettyCards_PackUcpPrice" pack_id="${pack_data.code_id}">10</span> UCP)</button>
	`;
}
...
if (settings.packs.value() && underscript.onPage('Packs')) {
	...
	loadCSSFromLink("https://cdn.jsdelivr.net/gh/CMD-God/prettycards/css/Packs.css");
